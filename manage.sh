...
status(){ echo; echo '========== legado-tts 服务状态 =========='; compose ps; echo; if curl -fsS --max-time 5 "http://127.0.0.1:${PORT}/health" >/tmp/legado-tts-health.$$ 2>/dev/null; then ok "HTTP 健康检查通过：$(cat /tmp/legado-tts-health.$$)"; rm -f /tmp/legado-tts-health.$$; else fail 'HTTP 健康检查失败，服务可能尚未启动或端口不可用'; rm -f /tmp/legado-tts-health.$$ || true; fi; }
